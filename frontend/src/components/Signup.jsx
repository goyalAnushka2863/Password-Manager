...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        toast('Signup successful! Please login.', { theme: 'dark' });
    ...
    <>
      ...
      <div className="absolute inset-0 -z-10 h-full w-full items-center px-5 py-24 [background:radial-gradient(125%_125%_at_50%_10%,#000_40%,#63e_100%)]">
        <div className="p-2 md:px-5 md:py-8 md:mycontainer overflow-auto mb-9 h-full max-w-md mx-auto">
          ...
          <p className='text-[#9e92f8fa] text-lg text-center mb-2'>Create a new account</p>
          <p className='text-center text-[#a59afffa] text-sm mb-4'>
            Already have an account? <Link to="/login" className='underline hover:text-[#6a42d8]'>Login</Link>
          </p>
          <form className="flex flex-col p-4 gap-8" onSubmit={handleSignup}>
            <input
